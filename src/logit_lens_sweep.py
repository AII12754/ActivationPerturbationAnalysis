"""
Sweep orchestration for logit lens experiments.
...
from .logit_lens_experiment import run_logit_lens_experiment
...
def build_logit_lens_sweep_jobs(config: Dict[str, Any]) -> List[ExperimentJob]:
    """Build the full list of logit lens experiment jobs from config."""
    sweep = config["sweep"]
    ...
    logger.info("Built %d logit lens sweep jobs (%d datasets).", len(jobs), len(dataset_specs))
    return jobs


def run_logit_lens_sweep(config: Dict[str, Any]):
    ...
    store = LogitLensResultStore(
        output_dir=store_cfg.get("output_dir", "./results_logit_lens"),
    ...
    # --- Build jobs ---------------------------------------------------
    all_jobs = build_logit_lens_sweep_jobs(config)
    model_id = model_cfg["path"]

    total = len(all_jobs)
    logger.info("Starting logit lens sweep: %d experiments.", total)
    t0 = time.time()
    ...
    for idx, job in enumerate(all_jobs):
        ...
        job_lens = dict(config.get("logit_lens", {}))
        job_config["logit_lens"] = job_lens

        try:
            perstep_records, topk_records = run_logit_lens_experiment(
                model=model,
                tokenizer=tokenizer,
        ...
        for rec in perstep_records:
            rec.update(metadata)
        for rec in topk_records:
            rec.update(metadata)

        store.add_perstep_records(job.job_id, perstep_records)
        store.add_topk_records(job.job_id, topk_records)
        store.mark_completed(job.job_id)

    store.flush()
    elapsed = time.time() - t0
    logger.info("Logit lens sweep complete. %d experiments in %.1f s.", total, elapsed)
...
def _parallel_worker(
    ...
    from .logit_lens_experiment import run_logit_lens_experiment as _run
    from .model import load_model_and_tokenizer
    from .prompts import PromptGenerator
    ...
    for idx, job in enumerate(jobs):
        ...
        try:
            perstep_records, topk_records = _run(
                model=model,
                tokenizer=tokenizer,
                prompt_text=prompt_text,
                config=job_config,
        ...
        results.append((job.job_id, {"perstep_records": perstep_records, "topk_records": topk_records}))
...
def run_logit_lens_sweep_parallel(config: Dict[str, Any]):
    ...
    if not gpus:
        ...
        return run_logit_lens_sweep(config)
    ...
    for job_id, result in results:
        if result and isinstance(result, dict) and "perstep_records" in result:
            store.add_perstep_records(job_id, result["perstep_records"])
            store.add_topk_records(job_id, result["topk_records"])
            store.mark_completed(job_id)

    store.flush()
    logger.info("Parallel logit lens sweep complete. %d jobs processed.", len(results))
