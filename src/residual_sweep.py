"""
Sweep orchestration for residual stream decomposition experiments.

Generates the Cartesian product of (dataset x context_length x prompt_index),
then dispatches experiments sequentially or in parallel. Each job runs both
prefill and decode phases (128 decode steps by default).
"""
...
from .residual_experiment import run_residual_experiment
...
def build_residual_sweep_jobs(config: Dict[str, Any]) -> List[ExperimentJob]:
    """Build the full list of residual experiment jobs from config."""
    sweep = config["sweep"]
    ...
    logger.info("Built %d residual sweep jobs (%d datasets).", len(jobs), len(dataset_specs))
    return jobs


def run_residual_sweep(config: Dict[str, Any]):
    ...
    store = ResidualResultStore(
        output_dir=store_cfg.get("output_dir", "./results_residual"),
    ...
    # --- Build jobs ---------------------------------------------------
    all_jobs = build_residual_sweep_jobs(config)
    model_id = model_cfg["path"]
    residual_cfg = config.get("residual", {})
    num_decode_tokens = residual_cfg.get("num_decode_tokens", 128)

    total = len(all_jobs)
    logger.info("Starting residual sweep: %d experiments.", total)
    t0 = time.time()
    ...
    for idx, job in enumerate(all_jobs):
        ...
        try:
            records = run_residual_experiment(
                model=model,
                tokenizer=tokenizer,
        ...
        store.add_records(job.job_id, records)
        store.mark_completed(job.job_id)

    store.flush()
    elapsed = time.time() - t0
    logger.info("Residual sweep complete. %d experiments in %.1f s.", total, elapsed)
...
def _parallel_worker(
    ...
    from .residual_experiment import run_residual_experiment as _run
    from .model import load_model_and_tokenizer
    from .prompts import PromptGenerator
    ...
    for idx, job in enumerate(jobs):
        ...
        results.append((job.job_id, {"records": records}))
...
def run_residual_sweep_parallel(config: Dict[str, Any]):
    ...
    if not gpus:
        ...
        return run_residual_sweep(config)
    ...
    for job_id, result in results:
        if result and isinstance(result, dict) and "records" in result:
            store.add_records(job_id, result["records"])
            store.mark_completed(job_id)

    store.flush()
    logger.info("Parallel residual sweep complete. %d jobs processed.", len(results))
