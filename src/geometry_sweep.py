"""
Sweep orchestration for representation geometry experiments.
...
from .geometry_experiment import run_geometry_experiment
...
def build_geometry_sweep_jobs(config: Dict[str, Any]) -> List[ExperimentJob]:
    """Build the full list of geometry experiment jobs from config."""
    sweep = config["sweep"]
    ...
    logger.info("Built %d geometry sweep jobs (%d datasets).", len(jobs), len(dataset_specs))
    return jobs


def run_geometry_sweep(config: Dict[str, Any]):
    ...
    store = GeometryResultStore(
        output_dir=store_cfg.get("output_dir", "./results_geometry"),
    ...
    # --- Build jobs ---------------------------------------------------
    all_jobs = build_geometry_sweep_jobs(config)
    model_id = model_cfg["path"]

    total = len(all_jobs)
    logger.info("Starting geometry sweep: %d experiments.", total)
    t0 = time.time()
    ...
    for idx, job in enumerate(all_jobs):
        ...
        try:
            summary_records, spectrum_records = run_geometry_experiment(
                model=model,
                tokenizer=tokenizer,
        ...
        store.add_summary_records(job.job_id, summary_records)
        store.add_spectrum_records(job.job_id, spectrum_records)
        store.mark_completed(job.job_id)

    store.flush()
    elapsed = time.time() - t0
    logger.info("Geometry sweep complete. %d experiments in %.1f s.", total, elapsed)
...
def _parallel_worker(
    ...
    from .geometry_experiment import run_geometry_experiment as _run
    from .model import load_model_and_tokenizer
    from .prompts import PromptGenerator
    ...
    for idx, job in enumerate(jobs):
        ...
        try:
            summary_records, spectrum_records = _run(
                model=model,
                tokenizer=tokenizer,
                prompt_text=prompt_text,
                config=config,
        ...
        results.append((job.job_id, {"summary_records": summary_records, "spectrum_records": spectrum_records}))
...
def run_geometry_sweep_parallel(config: Dict[str, Any]):
    ...
    if not gpus:
        ...
        return run_geometry_sweep(config)
    ...
    for job_id, result in results:
        if result and isinstance(result, dict) and "summary_records" in result:
            store.add_summary_records(job_id, result["summary_records"])
            store.add_spectrum_records(job_id, result["spectrum_records"])
            store.mark_completed(job_id)

    store.flush()
    logger.info("Parallel geometry sweep complete. %d jobs processed.", len(results))
