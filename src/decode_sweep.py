...
Generates the Cartesian product of (dataset × context_length × prompt_index ×
...
def _resolve_dataset_list(config: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    if not raw_list:
        ...
        return [{
            ...
            "config": prompt_cfg.get("dataset_config", "wikitext-103-raw-v1"),
...
def run_decode_sweep(config: Dict[str, Any]):
    ...
    for idx, job in enumerate(all_jobs):
        ...
        logger.info(
            "[%d/%d] Running %s — dataset=%s ctx=%d prompt=%d dec=%d",
            idx + 1,
            total,
            job.job_id,
            ds_label,
...
def _parallel_worker(
    ...
    from .decode_experiment import run_decode_experiment as _run
    from .model import load_model_and_tokenizer
    from .prompts import PromptGenerator
    ...
    for idx, job in enumerate(jobs):
        ...
        ds_label = os.path.basename(params["dataset_name"])
        _logger.info(
            "[%d/%d] GPU %s — %s dataset=%s ctx=%d prompt=%d dec=%d",
            idx + 1, total, gpu_indices, job.job_id,
            ds_label, params["context_length"],
            params["prompt_index"], params["num_decode_tokens"],
        )
        ...
        results.append((job.job_id, {"topk_records": topk_records, "agg_records": agg_records}))
...
def run_decode_sweep_parallel(config: Dict[str, Any]):
    ...
    for job_id, result in results:
        if result and isinstance(result, dict) and "topk_records" in result:
